...
import type { ISettingStorage, ISelected } from "@/utils/types";
import { devtools, persist } from "zustand/middleware";

const useSetting = create<ISettingStorage>()(
  devtools(
    persist(
      (set) => ({
        setting: null,
...
function SunIcon({ selected, ...props }: ISelected) {
  return (
    <svg
      viewBox="0 0 24 24"
      fill="none"
...
function MoonIcon({ selected, ...props }: ISelected) {
  return (
...
function PcIcon({ selected, ...props }: ISelected) {
  return (
    <svg viewBox="0 0 24 24" fill="none" {...props}>
      <path
