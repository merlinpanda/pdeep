import { ProseMirrorModel } from "@/components/prosemirror/api/prosemirror-model";
...
- <ProseMirrorModel /> provides the data structure that describes the editor's
  whole state, including the selection, and a transaction system for moving from
  one state to the next.
- <ProseMirrorModel /> implements a user interface component that shows a given
